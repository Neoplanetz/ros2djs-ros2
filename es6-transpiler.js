...
const transpile = {
  ...
  // Remove ROS2D prefix on internal dependencies
  internalDependencies: (filepath) => [
    // from:
    // return ROS2D.findClosestPoint(axisRay, mpRay);
    /(.*)ROS2D\.(\w+)(?!.*=.*)/g,
    // to:
  ...
  buildInheritanceIndexViaProto: [
    // from:
    // ROS2D.PoseWithCovariance.prototype.__proto__ = THREE.Object3D.prototype;
    /ROS2D\.(\w+)\.prototype\.__proto__ = (.*)\.prototype;[\r\n]?/g,
    // to:
  ...
  methods: [
    // from:
    // ROS2D.Arrow2.prototype.dispose = function () { ... };
    /ROS2D\.(\w+)\.prototype\.(\w+)\s*=\s*function\s*|function\s+(\w+)/g,
    // to:
    // dispose() { ... };
    (match, $1, $2, $3) => {
      if ($1) {
  ...
  constructors: (filepath, state = { foundConstructor: false }) => [
    // from:
    // ROS2D.Arrow2 = function(options) { ... };
    /ROS2D\.(\w+)\s*=\s*function\s*\((.*)\)/g,
    // to:
    // constructor(options) { ... };
    (match, $1, $2) => {
  ...
  superCalls: (filepath, state = { superMethodCalls: {} }) => [
    ...
    (match, indent, $1, $2) => {
      ...
      if (callee === parent) {
        ...
        if (!partsMatch) {
          logError('super method call invalid, parts don\'t match', { match, child, parent, callee, args })
          return match
        }
